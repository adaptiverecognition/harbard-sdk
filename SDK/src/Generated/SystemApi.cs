...
namespace Harbard
{
	namespace Api
	{
		...
		{
			public string? _Mode;
			public LocationSettings_Config? _Config;
			...
			public override JObject? serialize()
			{
				...
				_locationSettings.Add(new JProperty("Mode", _Mode));

				if(_Config != null)
				{ _locationSettings.Add(new JProperty("Config", _Config.serialize())); }
			...
			public override bool deserialize(JObject data)
			{
				...
				if(data_Mode != null)
				{ _Mode = Convert.ToString(data_Mode); }
				else
				{ success = false; }

				_Config = new LocationSettings_Config();
